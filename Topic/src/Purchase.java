...
public class Purchase implements ActionListener{
	...
	private static JButton purchase,refund,transfer,Previous_page = null;
	...
	public Purchase() {
		...
		panel.add(purchase);panel.add(refund);panel.add(transfer);panel.add(Previous_page);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==Previous_page) {
			new Main_page();
			Purchase.frame.dispose();
		}else if(e.getSource()==purchase) {
			split_time.setBottomComponent(new Purchase_TabbedPane().tabbedPane());  //必須兩個都設置  另半邊畫面會強制null
			split.setDividerLocation(150);
		}else if(e.getSource()==refund) {
			split_time.setBottomComponent(new Purchase_Refund().purchase_refund());  
			split.setDividerLocation(150);
