...
public class Purchase_Refund implements ActionListener{
	...
	public JSplitPane purchase_refund() {
		panel = new JPanel();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btn) {
			...
			try {
				String fileLoc = String.format("src/Info/Buy/%s",start);  //檔案路徑
		...
		if(e.getSource()==btn1) {
			...
			try {
				...
				file = String.format("src/Info/Buy/%s/%s",start,choose);    //用格式化選取檔案
