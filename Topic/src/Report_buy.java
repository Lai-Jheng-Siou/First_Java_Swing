...
public class Report_buy implements ActionListener{
	...
	public JSplitPane report_buy() {
		panel = new JPanel();
		
		lb = new JLabel("進貨紀錄:yyyy-MM-dd   開始時間:");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btn) {
			...
			String folderStart = String.format("src/Info/Buy/%s",start);
			...
			try {
				...
				columnNames.add("金額");columnNames.add("折扣");columnNames.add("備註");columnNames.add("時間");
				...
				for(File filelist:file.listFiles()) {
					...
					if(!filelist.isDirectory()) {
						String csv = String.format("src/Info/Buy/%s/%s", start,filelist.getName());
